...
import { MERMAID_TRANSFORMER } from '../MermaidTransformer';
...
export function setMarkdownConfig(config: Partial<MarkdownConfig & ListConfig>): void {
  if ('exportIndentSize' in config || 'importMinIndentSize' in config ||
      'importMaxIndentSize' in config || 'autoDetectIndent' in config) {
    ...
    if (!children && linesInBetween) {
      if (linesInBetween.length === 1) {
        ...
        if (linesInBetween[0].trim().length === 0) {
          while (linesInBetween.length > 0 && !linesInBetween[0].length) linesInBetween.shift();
        } else {
          linesInBetween[0] = linesInBetween[0].startsWith(' ') ? linesInBetween[0].slice(1) : linesInBetween[0];
        }
        while (linesInBetween.length > 0 && !linesInBetween[linesInBetween.length - 1].length) linesInBetween.pop();
...
export const BOLD_ITALIC_UNDERSCORE: TextFormatTransformer = UPSTREAM_BOLD_ITALIC_UNDERSCORE;
...
export const ITALIC_UNDERSCORE: TextFormatTransformer = UPSTREAM_ITALIC_UNDERSCORE;
...
export const LINK: TextMatchTransformer = {
  ...
  replace: (textNode, match) => {
    ...
    const linkUrl = rawUrl && rawUrl.startsWith('<') && rawUrl.endsWith('>')
      ? rawUrl.slice(1, -1)
      : rawUrl;
...
export const ELEMENT_TRANSFORMERS: Transformer[] = [HEADING, QUOTE, UNORDERED_LIST, ORDERED_LIST, TABLE_TRANSFORMER];

// MERMAID must come BEFORE CODE to intercept ```mermaid blocks first
export const MULTILINE_ELEMENT_TRANSFORMERS: Transformer[] = [MERMAID_TRANSFORMER, CODE];

export const TEXT_FORMAT_TRANSFORMERS: Transformer[] = [
  INLINE_CODE, BOLD_ITALIC_STAR, BOLD_ITALIC_UNDERSCORE,
  BOLD_STAR, BOLD_UNDERSCORE, HIGHLIGHT,
...
export { TABLE_TRANSFORMER } from '../plugins/TableTransformer';
