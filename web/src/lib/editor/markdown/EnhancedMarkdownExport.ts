...
type LexicalTransformer = ElementTransformer | MultilineElementTransformer | TextFormatTransformer | TextMatchTransformer;
...
export function $convertToEnhancedMarkdownString(
  ...
  return $exportNodeToMarkdown(transformers, $getRoot(), shouldPreserveNewLines);
}
...
function $exportNodeToMarkdown(
  ...
  const textFormatTransformers = byType.textFormat
    ...
    .sort((a, b) => Number(a.format.includes('code')) - Number(b.format.includes('code')));
  ...
  if (target && (!$isRootOrShadowRoot(target) || target.getType() === 'table')) {
    const result = exportTopLevelElements(
      target, elementTransformers, textFormatTransformers,
      textMatchTransformers, shouldPreserveNewLines,
    );
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      const result = exportTopLevelElements(
        child, elementTransformers, textFormatTransformers,
        textMatchTransformers, shouldPreserveNewLines,
      );
      if (result !== null) {
        output.push(
          isNewlineDelimited && i > 0 &&
          !isEmptyParagraph(child) && !isEmptyParagraph(children[i - 1])
            ? '\n' + result
            : result,
        );
      }
...
function exportTopLevelElements(
  ...
  for (const transformer of elementTransformers) {
    ...
    const result = transformer.export(node, (_node) =>
      exportChildren(_node, textFormatTransformers, textMatchTransformers,
        undefined, undefined, shouldPreserveNewLines, elementTransformers),
    );
  ...
  for (const child of children) {
    if ($isLineBreakNode(child)) {
      ...
      if (textTransformer) {
        ...
        if (hasFormatting) {
          output.push(exportTextFormat(child, textContentForTransform,
            textFormatTransformers, activeUnclosedTags, activeUnclosableTags, shouldPreserveNewLines));
          handled = true;
        } else {
          for (const transformer of textMatchTransformers) {
            ...
            const result = transformer.export(
              ...
              (_node: ElementNode, textContent?: string) =>
                exportChildren(_node, textFormatTransformers, textMatchTransformers,
                  textContent, textTransformer, shouldPreserveNewLines,
                  elementTransformers, activeUnclosedTags,
                  [...activeUnclosableTags, ...activeUnclosedTags]),
              (node: TextNode, textContent: string) =>
                exportTextFormat(node, textContent, textFormatTransformers,
                  activeUnclosedTags, activeUnclosableTags, shouldPreserveNewLines),
            );
            if (result != null) { output.push(result); handled = true; break; }
          }
        }
      ...
      for (const transformer of textMatchTransformers) {
        ...
        const result = transformer.export(
          ...
          (_node: ElementNode) =>
            exportChildren(_node, textFormatTransformers, textMatchTransformers,
              undefined, undefined, shouldPreserveNewLines,
              elementTransformers, activeUnclosedTags,
              [...activeUnclosableTags, ...activeUnclosedTags]),
          (node: TextNode, textContent: string) =>
            exportTextFormat(node, textContent, textFormatTransformers,
      ...
      if (!handled) {
        const result = exportTopLevelElements(child, elementTransformers || [],
          textFormatTransformers, textMatchTransformers, shouldPreserveNewLines);
        if (result != null) output.push(result);
      }
...
function exportTextFormat(
  ...
  if (!node.hasFormat('code')) {
    if (shouldPreserveNewLines) {
      ...
      output = output.replace(/\*/g, '&#42;').replace(/_/g, '&#95;').replace(/([`~])/g, '\\$1');
    } else {
  ...
  for (const transformer of textTransformers) {
    ...
    if (hasTextFormat(node, format) && !appliedFormats.has(format)) {
      ...
      if (!hasTextFormat(previousTextNode, format) ||
  ...
  for (let i = 0; i < unclosedTags.length; i++) {
    ...
    while (remainingTags.length > i) {
      ...
      if (tagToClose && unclosableTags.find((entry) => entry.tag === tagToClose.tag)) continue;
      if (tagToClose) {
...
function transformersByType(transformers: LexicalTransformer[]) {
  ...
  for (const transformer of transformers) {
    ...
    if (type === 'element') byType.element.push(transformer as ElementTransformer);
    else if (type === 'multiline-element') byType.multilineElement.push(transformer as MultilineElementTransformer);
    else if (type === 'text-format') byType.textFormat.push(transformer as TextFormatTransformer);
    else if (type === 'text-match') byType.textMatch.push(transformer as TextMatchTransformer);
  }
  return byType;
}
...
function getTextSibling(node: TextNode, backward: boolean): TextNode | null {
  ...
  if (!sibling) {
    ...
    if (parent?.isInline()) sibling = backward ? parent.getPreviousSibling() : parent.getNextSibling();
  }
  while (sibling) {
...
function hasTextFormat(node: LexicalNode | null | undefined, format: TextFormatType): boolean {
