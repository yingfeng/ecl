...
export function $convertFromEnhancedMarkdownString(
  ...
  const markdownWithPlaceholders = normalizedMarkdown.replace(MERMAID_REGEX, (_match, content) => {
  ...
  $convertFromMarkdownString(markdownWithPlaceholders, transformers || [], root, preserveNewLines);
 ...
 */
function $replacePlaceholders(root: ElementNode, mermaidContent: string[]): void {
