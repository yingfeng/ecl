...
const $createTableCell = (textContent: string): TableCellNode => {
  console.log('[TableTransformer] $createTableCell with:', JSON.stringify(textContent));
  textContent = textContent.replace(/\\n/g, '\n');
  const cell = $createTableCellNode(TableCellHeaderStates.NO_STATUS);
  $convertFromEnhancedMarkdownString(textContent, getTransformers(), cell, true);
  console.log('[TableTransformer] cell created, children:', cell.getChildrenSize());
  return cell;
};
...
const mapToTableCells = (textContent: string): Array<TableCellNode> | null => {
  ...
  const cells = match[1].split('|').map((text) => {
    ...
    console.log('[TableTransformer] cell text:', JSON.stringify(text), '->', JSON.stringify(trimmed));
